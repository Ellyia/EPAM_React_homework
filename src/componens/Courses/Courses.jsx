...
import { authors } from '../../store/authors/actionCreators';
...
const Courses = () => {
  ...
  useEffect(() => {
    (async () => {
      dispatch(authors(await loadAuthors()));
    })();
  }, []);
  ...
  const coursesList = useSelector((state) => {
    console.log('state.coursesReducer.courses.C', state.coursesReducer.courses); //
    return state.coursesReducer.courses;
  }, shallowEqual);

  const authorsList = useSelector((state) => {
    console.log('state.authorsReducer.authors.C', state.authorsReducer.authors); //
    const authors = state.authorsReducer.authors;
    return authors;
  }, shallowEqual);
